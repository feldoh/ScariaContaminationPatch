...
public static class JobGiver_GetFoodPatch
{
    ...
    public static Job TryGiveJob(Job __result, Pawn pawn)
    {
        ...
        if (prey != null)
        {
            ...
            if (prey.Spawned && prey.Faction == Faction.OfPlayer && prey.RaceProps.Humanlike && pawn.mindState != null)
            {
                pawn.mindState.enemyTarget = prey;
...
[HarmonyPatch(typeof(Hediff_Scaria), nameof(Hediff_Scaria.PostRemoved))]
public static class Hediff_Scaria_PostRemovedPatch
{
    [HarmonyPostfix]
    public static void PostRemoved(Hediff_Scaria __instance)
    {
        HediffComp_DevelopmentPointsOnRemoved.ApplyDevelopmentPointsIfRequired(__instance.pawn,
            ScariaContaminationPatch.Settings.ScariaPlayerFactionOnly,
            ScariaContaminationPatch.Settings.ScariaMustBeAlive,
            ScariaContaminationPatch.Settings.ScariaNonHostileOnly,
            ScariaContaminationPatch.Settings.ScariaBasePointsOnRemoved);
    }
}
